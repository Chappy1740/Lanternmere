...
let tokenError = false, calls = 0, tokenCalls = 0, responder;
function load(file, dependencies) {
  ...
  vm.runInNewContext(code, {
    exports, URL, AbortSignal, Date,
    require(name) {
      if (!(name in dependencies)) throw new Error('Unexpected dependency: ' + name);
      return dependencies[name];
    },
    fetch: async (...args) => { calls++; return responder(...args); },
  }, { filename: file });
  return exports;
}
...
async function failure(label, response, code, messagePattern, given = valid) {
  calls = 0; tokenCalls = 0; responder = response;
...
(async () => {
  ...
  ]) await failure('HTTP ' + status, () => new Response('SENSITIVE_SENTINEL', { status }), code, pattern);
  await failure('network failure', () => { throw new Error('SENSITIVE_SENTINEL'); }, 'integration', /could not be reached/i);
  await failure('timeout rejection', () => { throw new DOMException('SENSITIVE_SENTINEL', 'TimeoutError'); }, 'integration', /could not be reached/i);
  await failure('malformed JSON', () => new Response('not-json'), 'integration', /unexpected character profile/i);
  await failure('invalid profile shape', () => Response.json({ access_token: 'SENSITIVE_SENTINEL' }), 'integration', /unexpected character profile/i);
  tokenError = true;
  await failure('token failure prevents profile fetch', () => { throw new Error('must not fetch'); }, 'authentication', /Blizzard authentication/i);
  tokenError = false;
  ...
  responder = (url, options) => {
    ...
    return Response.json({ id: 1, name: 'Wrenx', level: 90,
      realm: { id: 1, name: 'Stormrage', slug: 'stormrage' },
      character_class: { id: 4, name: 'Rogue' }, race: { id: 1, name: 'Human' },
      faction: { type: 'ALLIANCE', name: 'Alliance' } });
  };
  const result = await fetchCharacterProfile({ region: ' US ', realm: ' STORMRAGE ', characterName: ' WRENX ' });
  assert.equal(result.ok, true); assert.equal(result.source, 'blizzard');
  assert.equal(result.region, 'us'); assert.ok(Number.isFinite(Date.parse(result.fetchedAt)));
  console.log('PASS: successful recovery, normalization, and timestamp'); passed++;
