-- Run this ENTIRE script only in lanternmere-security-test SQL Editor.
BEGIN;
-- Lanternmere pending-migration rehearsal. DRAFT: not database-executed.
-- Source commit: a4f392c
-- Migration SHA256: 52d24caaa45f8d515d41c5fc05614708cc980c40a3823e9905c2c2f42b4dec07
-- Use the SQL Editor in the disposable Supabase test database.
-- Requires all preceding repository migrations and privileged postgres access.
-- Run the WHOLE file; never execute selected fragments in an editor.
do $$ begin
  if current_user <> 'postgres' then raise exception 'Use the postgres test connection'; end if;
  if to_regprocedure('public.save_verified_wow_character(uuid,text,jsonb,timestamptz)') is not null
     or to_regprocedure('public.save_wow_character(text,jsonb,timestamptz)') is null then
    raise exception 'Expected pre-migration schema; stop and inspect migration history';
  end if;
end $$;
-- Baseline precedes the savepoint and survives rollback to that savepoint.
create temporary table lm_before as select md5(coalesce(string_agg(item, E'\n' order by item), '')) from (
  select 'table:' || c.oid::text || ':' || coalesce(c.relacl::text,'') || ':' || c.relrowsecurity::text || ':' || c.relforcerowsecurity::text as item
  from pg_class c where c.oid in ('public.characters'::regclass,'public.character_snapshots'::regclass)
  union all
  select 'column:' || a.attrelid::text || ':' || a.attnum::text || ':' || coalesce(a.attacl::text,'')
  from pg_attribute a where a.attrelid in ('public.characters'::regclass,'public.character_snapshots'::regclass) and a.attnum>0
  union all
  select 'policy:' || row_to_json(p)::text from pg_policies p
  where p.schemaname='public' and p.tablename in ('characters','character_snapshots')
  union all
  select 'function:' || pg_get_functiondef(p.oid) || ':' || coalesce(p.proacl::text,'')
  from pg_proc p join pg_namespace n on n.oid=p.pronamespace
  where (n.nspname='public' and p.proname in ('save_wow_character','save_verified_wow_character','set_character_lodge_sharing','set_main_character'))
     or (n.nspname='private' and p.proname='assign_character_main')
) definitions;
savepoint lm_before_migration;
set local lock_timeout = '5s';
set local statement_timeout = '30s';
set local idle_in_transaction_session_timeout = '60s';
-- BEGIN EXACT PENDING MIGRATION
...
-- Roll back migration and fixtures to the savepoint, verify baseline, then roll back the outer transaction.
rollback to savepoint lm_before_migration;
do $$
declare after_hash text;
begin
  select md5(coalesce(string_agg(item, E'\n' order by item), '')) into after_hash from (
  select 'table:' || c.oid::text || ':' || coalesce(c.relacl::text,'') || ':' || c.relrowsecurity::text || ':' || c.relforcerowsecurity::text as item
  from pg_class c where c.oid in ('public.characters'::regclass,'public.character_snapshots'::regclass)
  union all
  select 'column:' || a.attrelid::text || ':' || a.attnum::text || ':' || coalesce(a.attacl::text,'')
  ...
  if after_hash is distinct from (select md5 from lm_before) then
    raise exception 'FAIL: database definitions did not return to baseline';
  end if;
  if to_regclass('pg_temp.lm_fixture') is not null then
    raise exception 'FAIL: transaction fixture table still exists';
  end if;
  raise notice 'PASS: rollback restored function definitions, policies, grants, and RLS flags';
end $$;
drop table lm_before;

ROLLBACK;
SELECT 'REHEARSAL PASSED AND ROLLED BACK. Pending migration remains unapplied.' AS result;
