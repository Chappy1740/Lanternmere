...
-- Use a dedicated psql connection to a disposable Supabase test database.
...
\echo Starting rollback-only Lanternmere security rehearsal
do $$ begin
...
-- Session-local baseline survives the test transaction's rollback.
...
-- Roll back both the migration and all fixture writes.
rollback;
do $$
...
\echo REHEARSAL PASSED AND ROLLED BACK. Pending migration remains unapplied.
